...
import {
  ...
  WhatsApp,
  Info,
  AccountTree, // NOVO ÍCONE PARA TRIGGER FLOW
...
import TriggerFlowModal from "../TriggerFlowModal"; // NOVO MODAL

// ✅ CORREÇÃO KISS: Apenas otimizar configuração do MicRecorder para mobile
const Mp3Recorder = new MicRecorder({
  bitRate: 128,        // ✅ Reduzir bitrate para mobile (era 128)
  sampleRate: 44100   // ✅ 16kHz otimizado para voz
});

// ✅ CORREÇÃO KISS: Função simples para detectar mobile
...
const MessageInput = ({
  ...
  // NOVOS ESTADOS PARA TRIGGER FLOW MODAL
  ...
  const isAudioFile = (mediaPath, mediaType) => {
    if (mediaType === 'audio') return true;
    if (!mediaPath) return false;

    const audioExtensions = ['.mp3', '.wav', '.ogg', '.m4a', '.aac', '.webm'];
    const extension = mediaPath.substring(mediaPath.lastIndexOf('.')).toLowerCase();
          ...
          <div className={classes.newMessageBox}>
            ...
            {appointmentModalOpen && (
              <ScheduleModal
                ...
                message={inputMessage} // ✅ Passa a mensagem digitada
                contactId={contactId}   // ✅ Passa o ID do contato atual
                fromMessageInput={true} // ✅ Nova prop para identificar origem
              />
            )}
          </div>
        </Paper>
