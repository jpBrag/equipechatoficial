...
export const importXls = async (
  ...
  const profilePicPromise = GetProfilePicUrl(validNumber, companyId);
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const schema = Yup.object().shape({
    ...
    birthDate: Yup.date().nullable().max(new Date(), "Data de nascimento não pode ser no futuro"), // 🎂 VALIDAÇÃO BIRTHDATE
...
export const list = async (req: Request, res: Response): Promise<Response> => {
  ...
  const contacts = await SimpleListService({ name, companyId, userId: Number(userId) });
