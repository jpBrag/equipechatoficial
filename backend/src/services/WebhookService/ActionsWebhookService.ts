    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      if (nodeSelected.type === "openai") {
        try {
          ...
          if (!apiKey || !model) {
            logger.error(`[OPENAI NODE] Configurações obrigatórias não encontradas`);
            continue;
          }
          ...
          logger.info(`[OPENAI NODE] Configurando ticket ${ticket.id} para modo OpenAI (${flowMode})`);

          // ✅ Configurar ticket baseado no modo
          if (flowMode === "permanent") {
            // 🔄 MODO PERMANENTE: Para o fluxo (comportamento atual)
            await ticket.update({
              flowWebhook: false,
              lastFlowId: null,
              hashFlowId: null,
            ...
            logger.info(`[OPENAI NODE] Modo permanente ativado - fluxo pausado para ticket ${ticket.id}`);

            // Enviar boas-vindas e PARAR fluxo
            if (name) {
              const welcomeMessage = `Olá! Agora você está falando com ${name}. Como posso ajudá-lo?`;
              if (whatsapp.channel === "whatsapp") {
                await SendWhatsAppMessage({ body: welcomeMessage, ticket, quotedMsg: null });
              }
            }

            break; // ✅ PARAR FLUXO - PERMANENTE

          } else {
            // ⏳ MODO TEMPORÁRIO: Mantém informações do fluxo para voltar depois
            const nextConnection = connects.filter(
              connect => connect.source === nodeSelected.id && connect.sourceHandle === "a"
            )[0];

            await ticket.update({
              // ✅ MANTER informações do fluxo para continuar depois
              flowWebhook: true,
              lastFlowId: nodeSelected.id,
              hashFlowId: hashWebhookId,
              flowStopped: idFlowDb.toString(),

              // ✅ ATIVAR modo OpenAI temporário
              useIntegration: true,
              isBot: true,
              dataWebhook: {
                type: "openai",
                settings: openAiSettings,
                // ✅ GUARDAR informações para continuar fluxo
                flowContinuation: {
            ...
            logger.info(`[OPENAI NODE] Modo temporário ativado - fluxo será retomado para ticket ${ticket.id}`);

            // Enviar boas-vindas e PARAR fluxo temporariamente
            if (name) {
            ...
            break; // ✅ PARAR FLUXO - TEMPORÁRIO (ESTA LINHA ESTAVA FALTANDO!)
          }

        } catch (error) {
          logger.error(`[OPENAI NODE] Erro ao processar nó OpenAI:`, error);
          continue;
        }
      }
      ...
      if (nodeSelected.type === "singleBlock") {
        ...
        for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
          ...
          if (elementNowSelected.includes("message")) {
            ...
            if (whatsapp.channel === "whatsapp") {
              console.log("LINHA 379 - whatsapp", whatsapp.id, 'name', whatsapp.name)
              await SendMessage(whatsapp, {
                number: numberClient,
                body: msg,
                mediaPath: null
          ...
          if (elementNowSelected.includes("img")) {
            ...
            if (whatsapp.channel === "whatsapp") {
              console.log("LINHA 418 - whatsapp", whatsapp.id, 'name', whatsapp.name)
      ...
      if (nodeSelected.type === "menu") {
        // console.log(`[MENU NODE] Processando nó de menu ${nodeSelected.id}`);
        // console.log(`[MENU NODE] Message: ${nodeSelected.data.message}`);
        // console.log(`[MENU NODE] Options:`, nodeSelected.data.arrayOption);
        ...
        */
        ...
        if (pressKey) {
          ...
          //   undefined;
          if (execFn === undefined) {
            // console.log(`[MENU NODE] ExecFn undefined, parando fluxo`);
            break;
          }
          pressKey = "999";
