...
export const convertAudioToOgg = async (
  ...
  return new Promise<string>((resolve, reject) => {
    try {
      ...
      const outputFile = path.join(publicFolder, `company${companyId}`, newMediaFileName);
  ...
  try {
    ...
    if (ticket.isGroup) {
      ...
      try {
        ...
        if (ENABLE_LID_DEBUG) {
          logger.info(
            `[LID-DEBUG] Media - Sucesso na primeira tentativa para grupo ${jid}`
          );
        }
      } catch (err1) {
        if (ENABLE_LID_DEBUG) {
          logger.warn(
            `[LID-DEBUG] Media - Falha na primeira tentativa: ${err1.message}`
          );
        }
        ...
        if (err1.message && err1.message.includes("senderMessageKeys")) {
          if (ENABLE_LID_DEBUG) {
            logger.error(
              `[LID-DEBUG] Media - Erro de criptografia de grupo detectado: ${err1.message}`
            );
          }
          ...
          if (ENABLE_LID_DEBUG) {
            logger.info(
              `[LID-DEBUG] Media - Sucesso no envio sem criptografia para grupo ${jid}`
            );
          }
        } else {
          ...
          if (ENABLE_LID_DEBUG) {
            logger.info(
              `[LID-DEBUG] Media - Sucesso na segunda tentativa (sem contextInfo) para grupo ${jid}`
            );
          }
        }
      }
