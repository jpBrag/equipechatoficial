...
const checkLIDStatus = async (wbot: Session): Promise<boolean> => {
  try {
    ...
    // if (ENABLE_LID_DEBUG) {
    //   logger.info(
    //     `[LID-DEBUG] Verificando status do LID: ${
    //       isLIDEnabled ? "ATIVO" : "INATIVO"
    //     }`
    //   );
    //   logger.info(
    //     `[LID-DEBUG] User object: ${JSON.stringify(wbot.user, null, 2)}`
    //   );
    // }
    return !!isLIDEnabled;
  } catch (error) {
    // if (ENABLE_LID_DEBUG) {
    //   logger.info(`[LID-DEBUG] Erro ao verificar status do LID: ${error}`);
    // }
    return false;
  }
};
...
const verifyContact = async (
  ...
  const isLIDEnabled = await checkLIDStatus(wbot);

  if (ENABLE_LID_DEBUG) {
    logger.info(
      `[LID-DEBUG] LID está ${isLIDEnabled ? "ATIVO" : "INATIVO"} no WhatsApp`
    );
  }
  ...
  if (msg && msg.key) {
    if (ENABLE_LID_DEBUG) {
      logger.info(
        `[LID-DEBUG] Verificando key da mensagem: ${JSON.stringify(
          msg.key,
          null,
          2
        )}`
  ...
  // Verificar se o ID do contato é um LID
  else if (
    msgContact.id &&
    typeof msgContact.id === "string" &&
    msgContact.id.includes("@lid")
  ) {
    lid = msgContact.id;
    if (ENABLE_LID_DEBUG) {
      // logger.info(`[LID-DEBUG] LID encontrado em msgContact.id: ${lid}`);
    }
  }

  if (ENABLE_LID_DEBUG) {
    // logger.info(
    //   `[LID-DEBUG] Extraído lid da mensagem: '${lid}' para o contato ${msgContact.id}`
    // );
    logger.info(
      `[LID-DEBUG] Mensagem completa recebida: ${JSON.stringify(msg, null, 2)}`
    );

    // Log adicional para verificar todas as propriedades da key
    if (msg && msg.key) {
      // logger.info(`[LID-DEBUG] Todas as propriedades da key:`);
      Object.keys(msg.key).forEach(key => {
        const value = msg.key[key];
        // logger.info(`[LID-DEBUG]   ${key}: ${value} (tipo: ${typeof value})`);
      });
    }
  }
  ...
  const contactData = {
    name: msgContact.name || extractNumber(msgContact.id),
    number: extractNumber(msgContact.id),
    profilePicUrl,
    isGroup: msgContact.id.includes("g.us"),
    companyId,
    remoteJid: msgContact.id,
    lid, // Inclui o lid para unificação
    whatsappId: wbot.id,
    wbot,
...
const checkTemporaryOpenAI = async (wbot: Session, ticket: Ticket, contact: Contact, msgContact: IMe, mediaSent: Message, ticketTraking: TicketTraking, msg: proto.IWebMessageInfo) => {
  // Se o ticket tem dataWebhook com flowContinuation, pode estar em modo OpenAI temporário
  ...
  if (flowContinuation && ticket.useIntegration && ticket.flowStopped && ticket.lastFlowId) {
    try {
      // ✅ USAR A LÓGICA QUE JÁ EXISTE no código
      const flow = await FlowBuilderModel.findOne({
        where: {
          id: ticket.flowStopped
        }
      ...
      if (flow) {
        const lastFlowNode = flow.flow["nodes"].find(
          (node: any) => node.id === ticket.lastFlowId
        );

        if (lastFlowNode?.type === "openai") {
          const openAiConfig = lastFlowNode.data.typebotIntegration as IOpenAi;

          if (openAiConfig.flowMode === "temporary") {
            // Processar com as configurações do modo temporário
            await handleOpenAi(
              openAiConfig,
              msg,
              wbot,
      ...
      console.error("Erro ao verificar OpenAI temporário:", error);
    }
  }

  return false; // Não foi processado
};
...
const handleMessage = async (
  ...
  try {
    ...
    const wasProcessedByTemporaryOpenAI = await checkTemporaryOpenAI(wbot, ticket, contact, msgContact, null, ticketTraking, msg);
    if (wasProcessedByTemporaryOpenAI) {
      return; // Sair da função se foi processado pelo OpenAI temporário
    }

    // Verificar se este ticket está aguardando resposta de um nó de input
    // @ts-ignore
    if (
      !msg.key.fromMe &&
    ...
    // ✅ CORREÇÃO 1: Extrair TODOS os campos do OpenAI
    if (isOpenai && !isNil(flow) && !ticket.queue) {
      ...
      // ✅ EXTRAIR TODOS OS CAMPOS (básicos + novos)
      const {
        name,
        prompt,
        voice,
      ...
      // ✅ INCLUIR TODOS OS CAMPOS no openAiSettings
      openAiSettings = {
        name,
        prompt,
        voice,
        ...
        model,
        // ✅ NOVOS CAMPOS
        flowMode: flowMode || "permanent",
        maxInteractions: maxInteractions ? Number(maxInteractions) : undefined,
        continueKeywords: continueKeywords || [],
        completionTimeout: completionTimeout ? Number(completionTimeout) : undefined,
        objective: objective || "",
        autoCompleteOnObjective: autoCompleteOnObjective || false
      };
    ...
    if (
      ...
      await handleOpenAi(openAiSettings, msg, wbot, ticket, contact, mediaSent, ticketTraking);
    }
