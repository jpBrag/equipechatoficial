...
const UpdateUserService = async ({
  ...
  const {
    ...
    birthDate,
    allowSeeMessagesInPendingTickets // 🆕 INCLUIR NO DESTRUCTURING
  } = userData;
  ...
  try {
    await schema.validate({ 
      ...
      name, 
      birthDate,
      allowSeeMessagesInPendingTickets // 🆕 INCLUIR NA VALIDAÇÃO
    });
  } catch (err: any) {
  ...
  await user.update({
    ...
    allowSeeMessagesInPendingTickets // 🆕 INCLUIR NO UPDATE
  });
  ...
  const serializedUser = {
    ...
    allowSeeMessagesInPendingTickets: user.allowSeeMessagesInPendingTickets // 🆕 INCLUIR NO RETORNO
  };
